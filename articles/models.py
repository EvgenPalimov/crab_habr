...
class Category(models.Model):
    ...
    @staticmethod
    def choices():
        return [(blog.guid, blog.name) for blog in
...
class ArticleCategory(models.Model):
    ...
    article_guid = models.ForeignKey(Article, on_delete=models.CASCADE,
                                     null=False, verbose_name='Статья')
...
class ArticleLike(models.Model):
    ...
    event_counter = models.IntegerField(verbose_name='Счетчик', default=1,
                                        null=False)
    ...
    def get_like_type(article, user):
        try:
            obj = ArticleLike.objects.filter(article_uid=article,
                                             user_id=user).first()
            if obj:
    ...
    def get_like_rating(user):
        try:
            rating = ArticleLike.objects.filter(
                article_uid__in=[x.guid for x in Article.objects.filter(
    ...
    def set_like(article, user):
        ...
        if obj:
            ...
            ArticleLike.objects.create(article_uid=article, user_id=user,
                                       event_counter=1)
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=['user_id', 'article_uid'],
                                    name="%(app_label)s_%(class)s_unique")
        ]
...
class CommentLike(models.Model):
    ...
    date_added = models.DateField(default=timezone.now,
                                  verbose_name='Дата создания',
                                  db_column='dts')
    comment_uid = models.ForeignKey(Comment, verbose_name='Комментарий',
                                    on_delete=models.CASCADE)
    user_id = models.ForeignKey(User, verbose_name='Автор',
                                on_delete=models.SET_NULL, null=True)
    event_counter = models.IntegerField(verbose_name='Счетчик', default=1,
    ...
    def get_like_rating(user):
        try:
            rating = CommentLike.objects.filter(
                comment_uid__in=[x.guid for x in Comment.objects.filter(
    ...
    def get_like_type(comment, user):
        try:
            obj = CommentLike.objects.filter(comment_uid=comment,
                                             user_id=user).first()
            if obj:
                return "dislike" if obj.event_counter == 1 else "like"
            return "like"
    ...
    def set_like(comment, user):
        ...
        if obj:
            ...
            CommentLike.objects.create(comment_uid=comment, user_id=user,
                                       event_counter=1)
    ...
    class Meta:
        indexes = [models.Index(fields=['date_added'])]
        constraints = [
            models.UniqueConstraint(fields=['user_id', 'comment_uid'],
                                    name="%(app_label)s_%(class)s_unique")
        ]
...
class SubCommentLike(models.Model):
    ...
    def get_like_rating(user):
        try:
            rating = SubCommentLike.objects.filter(
                comment_uid__in=[x.guid for x in SubCommentLike.objects.filter(
                    user_id=user)]).aggregate(
                Sum('event_counter')).get('event_counter__sum', 0)
            return 0 if rating is None else rating
        except Exception as E:
    ...
    def get_like_type(comment, user):
        try:
            obj = SubCommentLike.objects.filter(comment_uid=comment,
                                                user_id=user).first()
            if obj:
                return "dislike" if obj.event_counter == 1 else "like"
            return "like"
    ...
    def set_like(comment, user):
        ...
        if obj:
            ...
            SubCommentLike.objects.create(comment_uid=comment, user_id=user,
                                          event_counter=1)
...
class Notification(models.Model):
    ...
    def __str__(self):
        return f'{self.date_added} от {self.author_id}: {self.message} для ' \
               f'{self.recipient_id}'
    ...
    def create_notification_like(sender, instance, **kwargs):
        notification = {'author_id': instance.user_id,
                        'recipient_id': instance.article_uid.author_id,
                        'message': f'поставил(а) лайк вашей статье - '
    ...
    def create_notification_like(sender, instance, **kwargs):
        notification = {'author_id': instance.user_id,
                        'recipient_id': instance.comment_uid.user_id,
    ...
    def create_notification_comment(sender, instance, **kwargs):
        notification = {'author_id': instance.user_id,
                        'recipient_id': instance.article_uid.author_id,
                        'article_uid': instance.article_uid,
                        'message': 'оставил(а) комментарий к вашей статье - '}
        new_notification = Notification(**notification)
        new_notification.save()
